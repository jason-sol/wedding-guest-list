...
exports.getUnusedCategoryColor = getUnusedCategoryColor;
...
function getCategoryColor(categoryName) {
    ...
    const index = Math.abs(hash) % exports.CATEGORY_COLORS.length;
    return exports.CATEGORY_COLORS[index];
}
// Get an unused color from the palette, or generate a new one if all are used
function getUnusedCategoryColor(existingColors) {
    // Find first unused color from palette
    for (const color of exports.CATEGORY_COLORS) {
    ...
    const baseColor = exports.CATEGORY_COLORS[existingColors.length % exports.CATEGORY_COLORS.length];
    return generateVariationColor(baseColor, existingColors.length);
}
// Generate a color variation when all palette colors are used
function generateVariationColor(baseColor, index) {
